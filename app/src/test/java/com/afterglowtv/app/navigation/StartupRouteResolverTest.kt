...
class StartupRouteResolverTest {
    ...
    @Test
    fun `vod container replaces movies and series in startup destinations`() {
        val visibleRoutes = StartupDestination.visibleEntries(developerModeEnabled = true).map { it.route }

        assertThat(visibleRoutes).contains(Routes.VOD_CONTAINER)
        assertThat(visibleRoutes).doesNotContain("movies")
        assertThat(visibleRoutes).doesNotContain("series")
    }
}

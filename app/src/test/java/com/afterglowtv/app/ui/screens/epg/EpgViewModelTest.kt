...
class EpgViewModelTest {
    private companion object {
        const val OLD_GUIDE_PAGE_SIZE = 60
    }
    ...
    fun setup() {
        ...
        runBlocking {
            ...
            whenever(preferencesRepository.adultCategorizedChannelCount(any())).thenReturn(flowOf(0))
            whenever(preferencesRepository.adultCategoryCache(any())).thenReturn(flowOf(null))
            whenever(preferencesRepository.adultSortBatchSize).thenReturn(flowOf(200))
            whenever(preferencesRepository.setAdultCategorizedChannelCount(any(), any())).thenReturn(Unit)
            whenever(preferencesRepository.setAdultCategoryCache(any(), any())).thenReturn(Unit)
            whenever(preferencesRepository.setAdultSortBatchSize(any())).thenReturn(Unit)
        }
        ...
        whenever(channelRepository.getChannelsWithoutErrors(any(), any())).thenReturn(flowOf(emptyList()))
    ...
    fun `guide loads programs with one batched repository call`() = runTest {
        ...
        whenever(channelRepository.getChannelsByCategoryPage(provider.id, ChannelRepository.ALL_CHANNELS_ID, OLD_GUIDE_PAGE_SIZE)).thenReturn(flowOf(channels))
        whenever(channelRepository.getChannelsWithoutErrorsPage(provider.id, ChannelRepository.ALL_CHANNELS_ID, OLD_GUIDE_PAGE_SIZE)).thenReturn(flowOf(channels))
    ...
    @Test
    fun `guide loads every channel instead of capping at first page`() = runTest {
        val provider = Provider(
            id = 1L,
            name = "Provider",
            type = ProviderType.M3U,
        ...
        val firstPage = channels.take(60)
        whenever(providerRepository.getActiveProvider()).thenReturn(flowOf(provider))
        whenever(preferencesRepository.getHiddenCategoryIds(provider.id, ContentType.LIVE)).thenReturn(flowOf(emptySet()))
        whenever(preferencesRepository.getCategorySortMode(provider.id, ContentType.LIVE)).thenReturn(flowOf(CategorySortMode.DEFAULT))
        whenever(parentalControlManager.unlockedCategoriesForProvider(provider.id)).thenReturn(flowOf(emptySet()))
        ...
        whenever(channelRepository.getChannelsByCategory(provider.id, ChannelRepository.ALL_CHANNELS_ID)).thenReturn(flowOf(channels))
        whenever(channelRepository.getChannelsWithoutErrors(provider.id, ChannelRepository.ALL_CHANNELS_ID)).thenReturn(flowOf(channels))
        whenever(channelRepository.getChannelsByCategoryPage(provider.id, ChannelRepository.ALL_CHANNELS_ID, OLD_GUIDE_PAGE_SIZE)).thenReturn(flowOf(firstPage))
        whenever(channelRepository.getChannelsWithoutErrorsPage(provider.id, ChannelRepository.ALL_CHANNELS_ID, OLD_GUIDE_PAGE_SIZE)).thenReturn(flowOf(firstPage))
        whenever(favoriteRepository.getFavorites(provider.id, ContentType.LIVE)).thenReturn(flowOf(emptyList()))
        whenever(preferencesRepository.guideDensity).thenReturn(flowOf(null))
        whenever(preferencesRepository.guideChannelMode).thenReturn(flowOf(null))
        whenever(preferencesRepository.guideDefaultCategoryId).thenReturn(flowOf(ChannelRepository.ALL_CHANNELS_ID))
        ...
        waitForUiState {
            viewModel.uiState.value.totalChannelCount == channels.size &&
                ...
                !viewModel.uiState.value.hasMoreChannels
        }
    ...
    fun `standard guide keeps broad sorting category labels visible`() = runTest {
        ...
        assertThat(viewModel.uiState.value.channels.map(Channel::name))
            ...
            .inOrder()
        assertThat(viewModel.uiState.value.categories.map(Category::name)).contains("Adult")
    ...
    fun `standard guide excludes adult category channels`() = runTest {
        ...
        assertThat(state.categories.map(Category::name)).contains("Adult")
        assertThat(state.categories.map(Category::name)).doesNotContain("XXX")
    }

    @Test
    fun `locked adult surface ignores saved epg category and favorites`() = runTest {
        val provider = Provider(
            id = 1L,
            name = "Provider",
            type = ProviderType.M3U,
        ...
        viewModel.applyNavigationContext(
            categoryId = com.afterglowtv.domain.model.VirtualCategoryIds.ADULT,
            anchorTime = null,
            favoritesOnly = false,
            lockCategory = true
        )
        ...
        advanceUntilIdle()
        waitForUiState {
            viewModel.uiState.value.selectedCategoryId == com.afterglowtv.domain.model.VirtualCategoryIds.ADULT &&
                viewModel.uiState.value.totalChannelCount == 1 &&
                !viewModel.uiState.value.isInitialLoading
        }
    ...
    @Test
    fun `locked adult surface waits for manual category chunks`() = runTest {
        val provider = Provider(
            id = 1L,
            name = "Provider",
            type = ProviderType.M3U,
        ...
        waitForUiState {
            val state = viewModel.uiState.value
            state.selectedCategoryId == com.afterglowtv.domain.model.VirtualCategoryIds.ADULT &&
        ...
        assertThat(firstState.adultCategorizedChannelCount).isEqualTo(0)
        assertThat(firstState.adultCategories).isEmpty()
        assertThat(firstState.isAdultCategorizing).isFalse()

        viewModel.categorizeNextAdultChunk()
        waitForUiStateWithRealDispatcher { viewModel.uiState.value.adultCategorizedChannelCount == 200 }

        val secondState = viewModel.uiState.value
        assertThat(secondState.adultCategorizedChannelCount).isEqualTo(200)
        assertThat(secondState.adultCategories.firstOrNull { it.title == "MILF" }?.channels?.size).isEqualTo(200)
        assertThat(secondState.adultCategories.map { it.key })
            .doesNotContain(com.afterglowtv.app.ui.model.AdultCategoryBuilder.ALL_CATEGORY_KEY)
        verify(preferencesRepository).setAdultCategorizedChannelCount(provider.id, 200)
        verify(preferencesRepository).setAdultCategoryCache(
            eq(provider.id),
        ...
        assertThat(reloadedState.adultCategorizedChannelCount).isEqualTo(200)
        assertThat(reloadedState.adultCategories.firstOrNull { it.title == "MILF" }?.channels?.size).isEqualTo(200)

        viewModel.categorizeNextAdultChunk()
        waitForUiStateWithRealDispatcher { viewModel.uiState.value.adultCategorizedChannelCount == 400 }

        val thirdState = viewModel.uiState.value
        assertThat(thirdState.adultCategorizedChannelCount).isEqualTo(400)
        assertThat(thirdState.adultCategories.firstOrNull { it.title == "MILF" }?.channels?.size).isEqualTo(400)

        viewModel.categorizeNextAdultChunk()
        waitForUiStateWithRealDispatcher { viewModel.uiState.value.adultCategorizedChannelCount == channels.size }

        val finalState = viewModel.uiState.value
        assertThat(finalState.adultCategorizedChannelCount).isEqualTo(channels.size)
        assertThat(finalState.adultCategories.firstOrNull { it.title == "MILF" }?.channels?.size).isEqualTo(channels.size)
        assertThat(finalState.adultCategories.firstOrNull { it.key == com.afterglowtv.app.ui.model.AdultCategoryBuilder.ALL_CATEGORY_KEY }?.channels?.size)
            .isEqualTo(channels.size)
    }

    @Test
    fun `locked adult surface restores saved category cache and time jumps do not reload channels`() = runTest {
        val provider = Provider(
            id = 1L,
            name = "Provider",
            type = ProviderType.M3U,
        ...
        whenever(favoriteRepository.getFavorites(provider.id, ContentType.LIVE)).thenReturn(flowOf(emptyList()))
        whenever(preferencesRepository.guideDefaultCategoryId).thenReturn(flowOf(ChannelRepository.ALL_CHANNELS_ID))
        whenever(preferencesRepository.adultCategoryCache(provider.id)).thenReturn(
            flowOf(
                AdultCategoryCache(
        ...
        waitForUiStateWithRealDispatcher {
            val state = viewModel.uiState.value
            state.selectedCategoryId == com.afterglowtv.domain.model.VirtualCategoryIds.ADULT &&
                state.adultCategorizedChannelCount == 200 &&
                state.adultCategories.firstOrNull { it.title == "MILF" }?.channels?.size == 200 &&
                !state.isAdultCategorizing &&
                !state.isInitialLoading &&
                !state.isRefreshing
        }
        ...
        val state = viewModel.uiState.value
        assertThat(state.adultCategorizedChannelCount).isEqualTo(200)
        assertThat(state.adultCategories.firstOrNull { it.title == "MILF" }?.channels?.map(Channel::id))
    ...
    @Test
    fun `locked adult surface uses configured manual sort batch size`() = runTest {
        val provider = Provider(
            id = 1L,
            name = "Provider",
            type = ProviderType.M3U,
        ...
        whenever(favoriteRepository.getFavorites(provider.id, ContentType.LIVE)).thenReturn(flowOf(emptyList()))
        whenever(preferencesRepository.guideDefaultCategoryId).thenReturn(flowOf(ChannelRepository.ALL_CHANNELS_ID))
        whenever(preferencesRepository.adultSortBatchSize).thenReturn(flowOf(400))
        ...
        val state = viewModel.uiState.value
        assertThat(state.adultSortBatchSize).isEqualTo(400)
        assertThat(state.adultCategories.firstOrNull { it.title == "MILF" }?.channels?.size).isEqualTo(400)
        verify(preferencesRepository).setAdultCategorizedChannelCount(provider.id, 400)
    }

    @Test
    fun `locked adult surface restores manual category progress from disk`() = runTest {
        val provider = Provider(
            id = 1L,
            name = "Provider",
            type = ProviderType.M3U,
        ...
        whenever(favoriteRepository.getFavorites(provider.id, ContentType.LIVE)).thenReturn(flowOf(emptyList()))
        whenever(preferencesRepository.guideDefaultCategoryId).thenReturn(flowOf(ChannelRepository.ALL_CHANNELS_ID))
        whenever(preferencesRepository.adultCategorizedChannelCount(provider.id)).thenReturn(flowOf(200))
        ...
        val state = viewModel.uiState.value
        assertThat(state.adultCategories.firstOrNull { it.title == "MILF" }?.channels?.size).isEqualTo(200)
        assertThat(state.adultCategories.map { it.key })
            .doesNotContain(com.afterglowtv.app.ui.model.AdultCategoryBuilder.ALL_CATEGORY_KEY)
    }
    ...
    fun `guide keeps no-data channels visible with blank schedule when schedule data is missing`() = runTest {
        ...
        whenever(channelRepository.getChannelsByCategoryPage(provider.id, ChannelRepository.ALL_CHANNELS_ID, OLD_GUIDE_PAGE_SIZE)).thenReturn(flowOf(listOf(channel)))
        whenever(channelRepository.getChannelsWithoutErrorsPage(provider.id, ChannelRepository.ALL_CHANNELS_ID, OLD_GUIDE_PAGE_SIZE)).thenReturn(flowOf(listOf(channel)))
        whenever(favoriteRepository.getFavorites(provider.id, ContentType.LIVE)).thenReturn(flowOf(emptyList()))
        whenever(preferencesRepository.guideDensity).thenReturn(flowOf(null))
        whenever(preferencesRepository.guideChannelMode).thenReturn(flowOf(null))
        whenever(preferencesRepository.guideDefaultCategoryId).thenReturn(flowOf(ChannelRepository.ALL_CHANNELS_ID))
    ...
    fun `scheduled current browse mode hides no-data channels without placeholders`() = runTest {
        ...
        whenever(favoriteRepository.getFavorites(provider.id, ContentType.LIVE)).thenReturn(flowOf(emptyList()))
        whenever(preferencesRepository.guideDensity).thenReturn(flowOf(null))
        whenever(preferencesRepository.guideChannelMode).thenReturn(flowOf(GuideChannelMode.ANCHORED.name))
        whenever(preferencesRepository.guideDefaultCategoryId).thenReturn(flowOf(ChannelRepository.ALL_CHANNELS_ID))
        whenever(preferencesRepository.guideFavoritesOnly).thenReturn(flowOf(false))
        whenever(preferencesRepository.guideScheduledOnly).thenReturn(flowOf(true))
        whenever(preferencesRepository.guideAnchorTime).thenReturn(flowOf(null))
        whenever(epgRepository.getResolvedProgramsForChannels(eq(provider.id), any(), any(), any())).thenReturn(emptyMap())
        whenever(epgRepository.getProgramsForChannelsSnapshot(eq(provider.id), eq(listOf("VOD Catalog Item")), any(), any())).thenReturn(emptyMap())
    ...
    fun `vod-like no-data channels keep blank guide cells instead of catalog placeholder block`() = runTest {
        ...
        val channel = Channel(
            ...
            name = "Catalog Movie",
            providerId = provider.id,
            epgChannelId = "Catalog Movie",
            categoryId = 10L,
            categoryName = "Movies",
            streamUrl = "https://provider.example.com/movie/12345.mp4?token=abc"
        )
        whenever(providerRepository.getActiveProvider()).thenReturn(flowOf(provider))
        whenever(preferencesRepository.getHiddenCategoryIds(provider.id, ContentType.LIVE)).thenReturn(flowOf(emptySet()))
        whenever(preferencesRepository.getCategorySortMode(provider.id, ContentType.LIVE)).thenReturn(flowOf(CategorySortMode.DEFAULT))
        whenever(preferencesRepository.guideNoDataBlockMinutes).thenReturn(flowOf(60))
        whenever(parentalControlManager.unlockedCategoriesForProvider(provider.id)).thenReturn(flowOf(emptySet()))
        whenever(channelRepository.getCategories(provider.id)).thenReturn(flowOf(listOf(Category(id = 10L, name = "Movies"))))
        whenever(channelRepository.getChannels(provider.id)).thenReturn(flowOf(listOf(channel)))
        whenever(channelRepository.getChannelsByCategory(provider.id, ChannelRepository.ALL_CHANNELS_ID)).thenReturn(flowOf(listOf(channel)))
        whenever(channelRepository.getChannelsWithoutErrors(provider.id, ChannelRepository.ALL_CHANNELS_ID)).thenReturn(flowOf(listOf(channel)))
        whenever(channelRepository.getChannelsByCategoryPage(provider.id, ChannelRepository.ALL_CHANNELS_ID, OLD_GUIDE_PAGE_SIZE)).thenReturn(flowOf(listOf(channel)))
        ...
        whenever(preferencesRepository.guideAnchorTime).thenReturn(flowOf(null))
        whenever(epgRepository.getResolvedProgramsForChannels(eq(provider.id), any(), any(), any())).thenReturn(emptyMap())
        whenever(epgRepository.getProgramsForChannelsSnapshot(eq(provider.id), eq(listOf("Catalog Movie")), any(), any())).thenReturn(emptyMap())
    ...
    fun `guide no-data blank text setting does not create placeholder chunks`() = runTest {
        ...
        whenever(preferencesRepository.guideNoDataTextMode).thenReturn(flowOf(GuideNoDataTextMode.BLANK))
        whenever(preferencesRepository.guideNoDataCustomText).thenReturn(flowOf(""))
        whenever(parentalControlManager.unlockedCategoriesForProvider(provider.id)).thenReturn(flowOf(emptySet()))
        whenever(channelRepository.getCategories(provider.id)).thenReturn(flowOf(listOf(Category(id = 10L, name = "Live"))))
        whenever(channelRepository.getChannels(provider.id)).thenReturn(flowOf(listOf(channel)))
    ...
    fun `guide updates provider metadata when active provider id stays the same`() = runTest {
        ...
        whenever(channelRepository.getChannelsByCategoryPage(originalProvider.id, ChannelRepository.ALL_CHANNELS_ID, OLD_GUIDE_PAGE_SIZE)).thenReturn(flowOf(emptyList()))
        whenever(channelRepository.getChannelsWithoutErrorsPage(originalProvider.id, ChannelRepository.ALL_CHANNELS_ID, OLD_GUIDE_PAGE_SIZE)).thenReturn(flowOf(emptyList()))
    ...
    fun `guide search finds matching program beyond initially loaded category page`() = runTest {
        ...
        whenever(channelRepository.getChannelsByCategoryPage(provider.id, ChannelRepository.ALL_CHANNELS_ID, OLD_GUIDE_PAGE_SIZE)).thenReturn(flowOf(listOf(loadedChannel)))
        whenever(channelRepository.getChannelsByCategoryPage(provider.id, 10L, OLD_GUIDE_PAGE_SIZE)).thenReturn(flowOf(listOf(loadedChannel)))
        whenever(channelRepository.getChannelsWithoutErrorsPage(provider.id, ChannelRepository.ALL_CHANNELS_ID, OLD_GUIDE_PAGE_SIZE)).thenReturn(flowOf(listOf(loadedChannel)))
        whenever(channelRepository.getChannelsWithoutErrorsPage(provider.id, 10L, OLD_GUIDE_PAGE_SIZE)).thenReturn(flowOf(listOf(loadedChannel)))
        whenever(favoriteRepository.getFavorites(provider.id, ContentType.LIVE)).thenReturn(flowOf(emptyList()))
        whenever(preferencesRepository.guideDensity).thenReturn(flowOf(null))
        whenever(preferencesRepository.guideChannelMode).thenReturn(flowOf(null))
        whenever(preferencesRepository.guideDefaultCategoryId).thenReturn(flowOf(ChannelRepository.ALL_CHANNELS_ID))
    ...
    fun `guide search by channel name preserves loaded schedule`() = runTest {
        ...
        whenever(channelRepository.getChannelsWithoutErrors(provider.id, 10L)).thenReturn(flowOf(channels))
        whenever(channelRepository.getChannelsByCategoryPage(provider.id, ChannelRepository.ALL_CHANNELS_ID, OLD_GUIDE_PAGE_SIZE)).thenReturn(flowOf(channels))
        whenever(channelRepository.getChannelsByCategoryPage(provider.id, 10L, OLD_GUIDE_PAGE_SIZE)).thenReturn(flowOf(channels))
        whenever(channelRepository.getChannelsWithoutErrorsPage(provider.id, ChannelRepository.ALL_CHANNELS_ID, OLD_GUIDE_PAGE_SIZE)).thenReturn(flowOf(channels))
        whenever(channelRepository.getChannelsWithoutErrorsPage(provider.id, 10L, OLD_GUIDE_PAGE_SIZE)).thenReturn(flowOf(channels))
        whenever(favoriteRepository.getFavorites(provider.id, ContentType.LIVE)).thenReturn(flowOf(emptyList()))
        whenever(preferencesRepository.guideDensity).thenReturn(flowOf(null))
        whenever(preferencesRepository.guideChannelMode).thenReturn(flowOf(null))
        whenever(preferencesRepository.guideDefaultCategoryId).thenReturn(flowOf(ChannelRepository.ALL_CHANNELS_ID))
    ...
    fun `guide prefers healthy channels but falls back when favorite channels have errors`() = runTest {
        ...
        whenever(parentalControlManager.unlockedCategoriesForProvider(provider.id)).thenReturn(flowOf(emptySet()))
        whenever(channelRepository.getCategories(provider.id)).thenReturn(flowOf(listOf(Category(id = 10L, name = "News"))))
        whenever(channelRepository.getChannelsByCategory(provider.id, ChannelRepository.ALL_CHANNELS_ID)).thenReturn(
        ...
        whenever(channelRepository.getChannelsByCategoryPage(provider.id, ChannelRepository.ALL_CHANNELS_ID, OLD_GUIDE_PAGE_SIZE)).thenReturn(
            flowOf(listOf(healthyChannel, unhealthyFavorite))
        )
        whenever(channelRepository.getChannelsWithoutErrorsPage(provider.id, ChannelRepository.ALL_CHANNELS_ID, OLD_GUIDE_PAGE_SIZE)).thenReturn(
            flowOf(listOf(healthyChannel))
        )
        whenever(channelRepository.getChannelsByCategoryPage(provider.id, 10L, OLD_GUIDE_PAGE_SIZE)).thenReturn(
            flowOf(listOf(healthyChannel, unhealthyFavorite))
        )
        whenever(channelRepository.getChannelsWithoutErrorsPage(provider.id, 10L, OLD_GUIDE_PAGE_SIZE)).thenReturn(
            flowOf(listOf(healthyChannel))
        )
    ...
    fun `guide favorites only loads favorite channels outside first category page`() = runTest {
        ...
        waitForUiState {
                viewModel.uiState.value.selectedCategoryId == 10L &&
                !viewModel.uiState.value.isRefreshing &&
                viewModel.uiState.value.channels.map { it.id } == listOf(firstPageChannel.id, favoriteOutsidePage.id)
        }
        viewModel.toggleFavoritesOnly()
        advanceUntilIdle()
        waitForUiState {
